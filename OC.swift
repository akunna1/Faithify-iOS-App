...
// from API show only the verses where Theme = OC

import SwiftUI

struct OC: View {
    @State private var OCVerses: [Verse] = [] // State variable to store OC-themed verses
                ...
                ZStack { // Start of ZStack
                    ...
                    VStack { // Start of VStack
                        if OCVerses.isEmpty {
                            ...
                            Text(OCVerses[currentVerseIndex].location)
                                .font(.system(size: 20))
                                .foregroundColor(Color.white) // text color
                                .bold()
                                .padding()
                            Text(OCVerses[currentVerseIndex].verse)
                                .font(.system(size: 20))
                                .foregroundColor(Color.white) // text color
                                .padding()
    ...
    // Function to load OC-themed verses from JSON
    private func loadVerses() {
        ...
        do {
            ...
            OCVerses = verses.filter { $0.theme == "Overcoming Challenges" } // Query
        } catch {
    ...
    // Function to shuffle OC-themed verses
...
struct OC_Previews: PreviewProvider {
