...
// from API show only the verses where Theme = Love

import SwiftUI

struct Love: View {
    @State private var loveVerses: [Verse] = [] // State variable to store love-themed verses
            ...
            NavigationView {
                ZStack { // Start of ZStack
                    ...
                    VStack { // Start of VStack
                        if loveVerses.isEmpty {
                            Text("No verses found")
                                .padding()
                        } else {
                            // Displaying location above the verse
                            Text(loveVerses[currentVerseIndex].location)
                                .font(.system(size: 20))
                                .foregroundColor(Color.white) // text color
                                .bold()
                                .padding()
                            Text(loveVerses[currentVerseIndex].verse)
                                .font(.system(size: 20))
                                .foregroundColor(Color.white) // text color
                                .padding()
    ...
    // Function to load love-themed verses from JSON
    private func loadVerses() {
        ...
        do {
            ...
            loveVerses = verses.filter { $0.theme == "Love" } // Query
        } catch {
    ...
    // Function to shuffle love-themed verses
